     ...
     district_state_level_disaggregation <- function(state_crop.db)
           {       
             ...
             # isolate modelled crops where TSI = 0
               ...
               if(!is.na(state_level_entry[1,7])) {to_distribute_tsi[,7] = state_level_entry[1,7]}
   ...
   #isolate state level entries
     state_level_disaggregation <- function(state_crop.db)
         ...
         # isolate modelled crops where TSI = 0
           ...
           to_distribute_tsi[,7] = state_level_entry[1,7]
    ...
    disaggregate_exposure <- function(Exposure.db, UserInput.db, Aggregate_user_exposure, district_state_level_disaggregation, district_level_disaggregation, state_level_disaggregation)
              {
                ...
                #  Initiate data
                   ...
                   for(i in 1:length(UI_CropSeasonID))
                      {
                        CS_id      =  UI_CropSeasonID[i]
                        ui.Crop.db <- UserInput.db[UserInput.db[,3] == CS_id,,drop=FALSE]
                        ex.Crop.db <- Exposure.db [Exposure.db [,3] == CS_id,,drop=FALSE]
                        ...
                        for(j in 1:length(UI_StateID))
                             {
                                ...
                                for(s in 1:nrow(tmp))
                                   {
                                     ...
                                     if(!is.na(ui.indemnity)){  if(ui.indemnity != 'Default'){tmp[s,9] <- ui.indemnity}}
                                     #if(!is.na(ui.indemnity)){  if(ui.indemnity == 'Default'){tmp[s,9] <- ui.indemnity}}
                                     #if(is.na(ui.indemnity)) {tmp[s,5] <- MNAIS.indemnity}
                                   }   

                                

                               #some times variable.x is generating NA's after merging, to insure it has the value
                                for(i in 1:nrow(tmp))
                                {
                                  if(is.na(tmp[i,1])) {tmp[i,1] <- tmp[i,14]} #district id
                                  if(is.na(tmp[i,3])) {tmp[i,3] <- tmp[i,6]} # crop season id
                                }
                              tmp = as.data.frame(tmp)
                               ...
                               #frame a array
                                ...
                                st_cr_district  = as.numeric(as.character(state_crop.db[,2]))
