...
#..........................................................................................................
# Duplicate
  Duplicated_entry_check <- function(UserInput_name_mismatch)
      {
         db_flag = 0
         state_levels <- unique(as.character(UserInput_name_mismatch$State_Name))

        for(i in 1:length(state_levels))
           {
              st_l                 = state_levels[i]
              state.db             = UserInput_name_mismatch[as.character(UserInput_name_mismatch[,2]) == st_l,]
              district_levels      = unique(as.character(UserInput_name_mismatch$District_Name))

              for(j in 1:length(district_levels))
                 {
                    ds_l            = district_levels[j]
                    district.db     = state.db[as.character(state.db[,3]) == ds_l,]
                    crop_levels     = unique(as.character(district.db$Crop))

                 for(k in 1:length(crop_levels))
                    {
                       cr_l          = crop_levels[k]
                       crop.db       = district.db[as.character(district.db[,4]) == cr_l,]
                       season_levels = unique(as.character(district.db$Season))

                       for(l in 1:length(season_levels))
                           {
                             sn_l      = season_levels[l]
                             season.db = crop.db[as.character(crop.db[,5]) == sn_l,]
                             if(nrow(season.db) > 1){season.db[,10:11] <- 'Duplicated Entry'}

                             if(db_flag == 1){final.db <- rbind(final.db, season.db)}
                             if(db_flag == 0){final.db <- season.db; db_flag = 1}
          }     }    }     }
...
# Check if districts and states are modelled
  Check_UserInput_modelled_adminlevel<- function(UserInput.db, Exposure.db)
     ...
     # put in flags
      ...
      UserInput.db = ui[,c(-10,-11,-12,-13),drop=FALSE]
  ...
  Check_UserInput_TSI_check <- function(UserInput.db)
         {   
          ...
          #  UserInput.db = UserInput_is_modelled
            ...
            for(m in 1:length(season_levels))
             {
               sn_l      = season_levels[m]
               season.db = UserInput.db[as.character(UserInput.db[,5]) == sn_l,,drop = FALSE]

               crop_levels = unique(as.character(season.db[,4]))
               for(n in 1:length(crop_levels))
                  {
                    cr_l    = crop_levels[n]
                    crop.db = season.db[as.character(season.db[,4]) == cr_l,,drop = FALSE]

                    state_levels <- unique(as.character(crop.db[,2]))

                    for(s in 1:length(state_levels))
                       {
                         st_l     = state_levels[s]
              ...
              for(i in 1:nrow(UserInput.db))
                  {
                    ...
                    if(IL != 'Default')
                      {
                        options(warn=-1);IL = as.numeric(IL);options(warn=0)
                        
                        if(is.na(IL))    {UserInput.db[i,9:10] = 'Indemnity Level entry does not comply, Default will be taken'}
                        if(!is.na(IL))
                           {
                              if(IL > 1) {UserInput.db[i,9:10] = 'Indemnity Level is greater than 1, Default will be taken'}
                              if(IL < 0) {UserInput.db[i,9:10] = 'Indemnity Level is less than 0, Default will be taken'   }
                           }
...
# Check for any duplicated entries
   ...
   Check_Duplicates <-function(duplicate_check)
          {
             # Check for any duplicated entries
               df <- data.frame(duplicate_check[,2:5])
               x  =  duplicated(df) | duplicated(df[nrow(df):1, ])[nrow(df):1]
               Duplicated_UI_check = cbind(duplicate_check[,1:9],x,x)

             # allocated checks
               Duplicated_UI_check[(as.character(Duplicated_UI_check[,10])) == 'TRUE', 10] <- 'Duplicated Entry'
               Duplicated_UI_check[(as.character(Duplicated_UI_check[,11])) == 'TRUE', 11] <- 'Duplicated Entry'

               Duplicated_UI_check[(as.character(Duplicated_UI_check[,10])) == 'FALSE', 10] <- 'Good'
               Duplicated_UI_check[(as.character(Duplicated_UI_check[,11])) == 'FALSE', 11] <- 'Good'
             
               return(Duplicated_UI_check)
          }
#..........................................................................................................
  ...
  Check_UserInput <- function(UserInput.db,adminID.db,Exposure.db,Product_type.db, Check_UserInput_Name_Mismatch,  Check_UserInput_Prepare_Exposure_db, Check_UserInput_modelled_adminlevel, Check_UserInput_TSI_check)
       {      
         ...
         # Check for any duplicated entries
              ...
              duplicate_check = UserInput_name_mismatch[(as.character(UserInput_name_mismatch[,10])) == 'Good', ]
              UserInput_duplicate <- Check_Duplicates(duplicate_check)
         #..........................................................................................................
         ...
         #..........................................................................................................
               ...
               UserInput_duplicate     = as.data.frame(UserInput_duplicate)
         ...
         # combine and prioritise check messages
               ...
               for(i in 1:rowL)
                 {
                    ...
                    is_duplicate        <- as.matrix(UserInput_duplicate[as.numeric(as.character(UserInput_duplicate[,1])) == i,,drop = FALSE])
                    if(nrow(is_duplicate) > 0)
                         {
                          MNAIS_is_duplicate   <- as.character(is_duplicate[1,10,drop=FALSE])
                          WBCIS_is_duplicate   <- as.character(is_duplicate[1,11,drop=FALSE])
                         }
                      ...
                      if(nrow(is_duplicate)> 0){ if(MNAIS_is_duplicate != 'Good'){name_mismatch[1,10] <- as.character(MNAIS_is_duplicate)}}
                      ...
                      if(nrow(is_duplicate)> 0){ if(WBCIS_is_duplicate != 'Good'){name_mismatch[1,11] <- as.character(WBCIS_is_duplicate)}}
              ...
              final_checked_userinput.db = as.data.frame(final.ui[,-1,drop = FALSE])
